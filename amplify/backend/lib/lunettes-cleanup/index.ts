...
async function deleteCompletedCampaigns() {
    ...
    for await (const campaign of api.getCampaigns(value => [CampaignStatus.DELETED.toString(), CampaignStatus.COMPLETED.toString()].includes(
        value.State.CampaignStatus.toString()) && value.Schedule && value.Schedule.Frequency === Frequency.ONCE && DateTime.fromISO(
